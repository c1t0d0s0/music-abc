...
著作権の保護期間が満了した唱歌、各国の国歌、クラシックの名旋律など 32 曲を収録しています。
...
- 日本語・英語表示（ブラウザの第一言語が日本語なら日本語、それ以外は英語。`src/i18n.ts`）
...
プロジェクト直下に `config.js` を置き、Google アナリティクス 4 の測定 ID（`G-` で始まる）または Google タグマネージャーのコンテナ ID（`GTM-` で始まる）を書くと、`npm run build` のときに計測タグが埋め込まれます。

```js
const GTM_ID = 'G-XXXXXXXXXX';
```

- `config.js` は `.gitignore` に入っていて、リポジトリには含めません。ファイルがない・ID が空のときはタグを入れません。
- 環境変数 `GA_ID`（または `GTM_ID`）で指定することも可能です（GitHub Actions の `vars.GA_ID` など CI ビルドで利用できます）。
- 開発サーバー（`npm run dev`）では計測しません。
- ページを移動するたびに、`/song/furusato` のような URL でページビューを送ります（ルーティングがハッシュ方式のため）。GA4 の「拡張計測機能」の「ブラウザの履歴イベントに基づくページの変更」はオフにしてください。オンのままだとページビューが重複します。
- ID があるときは、「ライセンス」ページにアクセス解析についての説明が表示されます。

## 曲を追加するには

1. `src/songs/<カテゴリ>/<id>.abc` を作る（`school` / `anthems` / `classical-folk`）
2. `src/songs/meta.ts` の `SONGS` に、同じ `id` で作者・没年・発表年などを追加する。曲名・作者名・注記は日本語と英語の両方（`{ ja, en }`）で書く。英語表示のときは、譜面の曲名と作者の行がこのデータから作られる
